...
function dynamichtml(result, id) {
    ...
    $.each(subtrip, function (index, value) {
        ...
        if(websetting.luggage_service == 1){
            ...
            if(value.free_luggage_kg == null){
                value.free_luggage_kg = 0.00;
            }
            if(value.paid_max_luggage_pcs == null){
                value.paid_max_luggage_pcs = 0;
            }
            if(value.paid_max_luggage_kg == null){
                value.paid_max_luggage_kg = 0.00;
            }
            if(value.price_pcs == null){
                value.price_pcs = 0.00;
            }
            if(value.price_kg == null){
                value.price_kg = 0.00;
            }

            free_luggage_pcs = (parseInt(value.free_luggage_pcs) != 0) ? value.free_luggage_pcs  : luggagesetting.free_luggage_pcs;
            free_luggage_kg = (parseFloat(value.free_luggage_kg) != 0.00) ? value.free_luggage_kg : luggagesetting.free_luggage_kg;
            paid_max_luggage_pcs =(parseInt(value.paid_max_luggage_pcs) != 0)  ? value.paid_max_luggage_pcs:  luggagesetting.paid_max_luggage_pcs;
            paid_max_luggage_kg = (parseFloat(value.paid_max_luggage_kg) != 0.00) ? value.paid_max_luggage_kg: luggagesetting.paid_max_luggage_kg;
            price_pcs = (parseFloat(value.price_pcs) != 0.00) ? value.price_pcs : luggagesetting.price_pcs;
            price_kg = (parseFloat(value.price_kg) != 0.00) ? value.price_kg : luggagesetting.price_kg;
        }
    ...
    $('input', $adultSeatCount)
        ...
        $pricePcs
            .addClass('col-6 mt-2')
            .append([$pricePcsLbl, $pricePcsInp]);
    ...
    if(websetting.luggage_service == 1){
    html += $freeLuggagePcs.prop('outerHTML') + $freeLuggageKg.prop('outerHTML') +
    $paidMaxLuggagePcs.prop('outerHTML') + $paidMaxLuggageKg.prop('outerHTML') +
    $pricePcs.prop('outerHTML') + $priceKg.prop('outerHTML')+ $specialLuggage.prop('outerHTML');
    }
    ...
    html += `   <h6 class="form-label"> ${bdtaskIlmCommonJs.lang.getPhrase('free')} ${bdtaskIlmCommonJs.lang.getPhrase('luggage')} ( <span class="form-label" id="totalfreeluggagepcs${id}"> 0 </span> pcs ) |  ( <span class="form-label" id="totalfreeluggagekg${id}">0</span> kg ) </h6>`;
    html += '</div>';
    ...
    html += '<div class="col-md-4">';
    html += `   <h6 class="form-label"> ${bdtaskIlmCommonJs.lang.getPhrase('paid')} ${bdtaskIlmCommonJs.lang.getPhrase('luggage')} ( <span class="form-label" id="totalpaidluggagepcscount${id}"> 0 </span> pcs )</h6>`;
    html += '</div>';
    ...
    html += '<div class="col-md-4 ">';
    html += '   <h6 class="form-label" id="totalpaidluggagepcs' + id + '">0</h6>';
    html += '</div>';

    html += '<div class="col-md-4">';
    html += `   <h6 class="form-label"> ${bdtaskIlmCommonJs.lang.getPhrase('paid')} ${bdtaskIlmCommonJs.lang.getPhrase('luggage')} ( <span class="form-label" id="totalpaidluggagekgcount${id}">0</span> kg )</h6>`;
    html += '</div>';
    ...
    html += '<div class="col-md-4 ">';
    html += '   <h6 class="form-label" id="totalpaidluggagekg' + id + '">0</h6>';
    html += '</div>';
    }
    ...
    html += '<div class="col-md-4">';
    html += `   <h6 class="form-label">${bdtaskIlmCommonJs.lang.getPhrase('sub')} ${bdtaskIlmCommonJs.lang.getPhrase('total')}</h6>`;
    html += '</div>';
...
function freeLagguageKG(cseat, subtripid) {
    var $field = $(cseat),
        ...
        originalValue = parseFloat($field.val()) || 0.00;
...
function toprice(subtripid) {
    ...
    var subbtotal = 0;
    if(websetting.luggage_service == 1){
        subbtotal = totalprice + parseFloat(totalLuggagePricePcs) + parseFloat(totalLuggagePriceKg);
    }else{
...
function taxcaltulation(subtripid, totalvalue) {
    ...
    // if(websetting.luggage_service == 1){
    //     grandtotal = grandtotal + parseFloat(totalLuggagePricePcs) + parseFloat(totalLuggagePriceKg);
    // }else{
...
function formsubmit(index, subtripid) {
    ...
    if(websetting.luggage_service == 1){
        ...
        free_luggage_kg = parseFloat($("#free_luggage_kg" + subtripid).val()).toFixed(2),
        paid_max_luggage_pcs = parseInt($("#paid_max_luggage_pcs" + subtripid).val()),
        paid_max_luggage_kg = parseFloat($("#paid_max_luggage_kg" + subtripid).val()).toFixed(2),
